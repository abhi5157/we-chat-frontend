...
function Sidebar({ onSelectUser, activeUserId, users, currentUserId }) {
  ...
  return (
    <div>
      {!isSidebarOpen && (
        <button
          className="lg:hidden fixed top-12 left-4 z-20 text-white bg-blue-500 p-2 rounded-full"
      ...
      >
        <div className="p-4 sticky top-0 bg-white z-10">
          ...
          {filteredUsers.length === 0 && searchTerm.trim() === "" && (
            <h2 className="text-xl font-bold text-gray-800 py-20">
            ...
            >
              <div className="flex items-center">
                <div className="w-12 h-12 rounded-full mr-3 bg-gray-300 flex items-center justify-center">
                  {user.avatar ? (
                    <img
                      src={`http://http://autorack.proxy.rlwy.net:12882/${user.avatar}`}
                      alt={user.firstName}
                    />
                  ) : (
